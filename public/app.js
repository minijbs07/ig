...
function showAlert(elId, msg, type = 'info') {
  ...
  if (!el) return;
  el.innerHTML = `<div class="alert alert-${type}" style="padding:1rem;border-radius:4px;margin-bottom:1rem;background:${type==='error'?'rgba(229,57,53,.1)':type==='success'?'rgba(76,175,80,.1)':'rgba(212,175,55,.1)'};border:1px solid ${type==='error'?'rgba(229,57,53,.4)':type==='success'?'rgba(76,175,80,.4)':'rgba(212,175,55,.4)'};color:${type==='error'?'#e57373':type==='success'?'#81c784':'var(--gold)'}">${msg}</div>`;
}
...
function initReservas() {
  ...
  loadScheduleSidebar();
  loadPublicBookings();
  dateInput.addEventListener('change', () => { if (dateInput.value) loadTimeSlots(dateInput.value); });
  form.addEventListener('submit', async e => {
    ...
    if (!name || !email || !service || !date || !time) {
      showAlert('bookingAlert', 'Por favor completa todos los campos obligatorios.', 'error'); return;
    }
    ...
    try {
      ...
      btn.innerHTML = 'Confirmar Reserva'; btn.disabled = false;
    }
  });
}
...
async function loadTimeSlots(date) {
  ...
  } catch { sel.innerHTML = '<option value="">Error al cargar horarios</option>'; }
}
...
async function loadScheduleSidebar() {
  ...
  } catch { el.innerHTML = '<li><span class="day">No disponible</span></li>'; }
}
  ...
  try {
    ...
    if (upcoming.length === 0) {
      ...
      chipsEl.innerHTML = upcoming.map(b => {
        ...
        return `<span class="booking-chip color-${ci}" title="${b.service} - ${formatDate(b.date)} ${b.time}">${b.name} - ${b.time} ${formatDate(b.date).slice(0,6)}</span>`;
      }).join('');
    }
  } catch { if (chipsEl) chipsEl.innerHTML = '<div style="color:var(--grey);font-size:.85rem;text-align:center">Servidor no disponible</div>'; }
}
...
function initAdmin() {
  ...
  document.getElementById('loginForm').addEventListener('submit', async e => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok && data.token) { sessionStorage.setItem(ADMIN_KEY, data.token); showAdminPanel(); }
      else showAlert('loginAlert', 'Contrasena incorrecta.', 'error');
    } catch { showAlert('loginAlert', 'Error de conexion con el servidor.', 'error'); }
  });
...
async function initAdminPanel() {
  ...
  initScheduleSave();
  initConfigSave();
  document.getElementById('filterDate').addEventListener('change', e => { filterDate = e.target.value; renderBookingsTable(); });
}
...
function initFilterButtons() {
  document.querySelectorAll('.filter-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      btn.classList.add('active'); currentFilter = btn.dataset.filter; renderBookingsTable();
    });
  });
}
...
async function loadAdminBookings() {
  try {
    ...
    updateSummaryCards(); renderBookingsTable();
  } catch {
...
function renderBookingsTable() {
  ...
  if (filtered.length === 0) { tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:3rem;color:var(--grey)">No hay reservas con estos filtros</td></tr>'; return; }
  const statusBadge = s => { const map={pending:'badge-pending',confirmed:'badge-confirmed',cancelled:'badge-cancelled'}; const labels={pending:'Pendiente',confirmed:'Confirmada',cancelled:'Cancelada'}; return `<span class="badge ${map[s]||'badge-pending'}">${labels[s]||s}</span>`; };
  tbody.innerHTML = filtered.map(b => `
    <tr data-id="${b.id}">
      ...
      <td style="font-size:.8rem;color:var(--grey);max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${b.notes ? escHtml(b.notes) : '-'}</td>
      <td>
...
async function updateBookingStatus(id, status) {
  try {
    const res = await fetch(`${API}/api/bookings/${id}`, { method: 'PATCH', headers: authHeaders(), body: JSON.stringify({ status }) });
    if (res.ok) { showToast(status === 'confirmed' ? 'Reserva confirmada' : 'Reserva cancelada', 'success'); await loadAdminBookings(); }
...
async function deleteBooking(id) {
  ...
  try {
    const res = await fetch(`${API}/api/bookings/${id}`, { method: 'DELETE', headers: authHeaders() });
    if (res.ok) { showToast('Reserva eliminada', 'info'); await loadAdminBookings(); }
...
async function loadAdminSchedule() {
  try {
    ...
    renderScheduleGrid();
  } catch { scheduleData = getDefaultSchedule(); renderScheduleGrid(); }
}
  ...
  grid.innerHTML = scheduleData.map((d, i) => `
    <div class="schedule-day-card">
      ...
      <div class="day-toggle">
        <label class="toggle-switch"><input type="checkbox" id="open_${i}" ${d.open ? 'checked' : ''} onchange="toggleDay(${i})"><span class="toggle-slider"></span></label>
...
function initScheduleSave() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch(`${API}/api/schedule`, { method: 'POST', headers: authHeaders(), body: JSON.stringify({ schedule: scheduleData }) });
...
function initConfigSave() {
  ...
  btn.addEventListener('click', async () => {
    ...
    const maxDaysAhead = parseInt(document.getElementById('maxDaysAhead').value) || 30;
    const minNotice = parseInt(document.getElementById('minNotice').value) || 60;
    const newPassword = document.getElementById('newPassword').value;
    const payload = { slotDuration, maxDaysAhead, minNotice };
    if (newPassword) payload.newPassword = newPassword;
    try {
      const res = await fetch(`${API}/api/config`, { method: 'POST', headers: authHeaders(), body: JSON.stringify(payload) });
      if (res.ok) { showToast('Configuracion guardada', 'success'); document.getElementById('newPassword').value = ''; }
      else showToast('Error al guardar configuracion', 'error');
    } catch { showToast('Error de conexion', 'error'); }
  });
}
